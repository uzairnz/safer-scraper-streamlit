...
def main():
    ...
    usdot_code_finish = st.number_input("Enter Finishing US DOT Code", value=0, step=1, format="%d")
    
    if usdot_code_start > usdot_code_finish:
        st.warning("Starting DOT code should be less than or equal to Finishing DOT code.")
        return
    
    if st.button("Fetch and Export Data"):
        data = []
        for i in range(usdot_code_start, usdot_code_finish + 1):
            try:
